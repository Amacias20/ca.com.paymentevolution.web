...
const AbsenceTypesContainer = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await AbsenceTypeService.getAbsenceTypes(currentPage, page.take);
      const payload = response.data?.data;
      let rows: any[] = payload?.data ?? payload?.items ?? [];
      if (applied.name) rows = rows.filter(r => r.name?.toLowerCase().includes(applied.name.toLowerCase()));
    ...
    } catch { toastError(t('absenceTypes.title') + ': error loading data'); }
    finally { setLoading(false); }
  }, [page, applied, t]);

  useEffect(() => { fetchData(); }, [fetchData]);

  const handleSearch = () => { setPage({ skip: 0, take: PAGE_SIZE }); setApplied({ name: filterName, status: filterStatus.value }); };
  const handleClear = () => { setFilterName(''); setFilterStatus(STATUS_OPTIONS[0]); setApplied({ name: '', status: '' }); setPage({ skip: 0, take: PAGE_SIZE }); };
  const handleEdit = (item: any) => navigate(`/absence-types/${item.idAbsenceType ?? item.absenceTypeId ?? item.id}/edit`);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await AbsenceTypeService.deleteAbsenceType(itemToDelete.idAbsenceType ?? itemToDelete.absenceTypeId ?? itemToDelete.id);
    ...
    <div className="page-content animate-fade-in">
      <div className="page-header-card">
        <div className="page-header-card-top">
          <div>
            <div className="page-title"><div className="page-title-icon">📋</div>{t('absenceTypes.title')}</div>
            <p className="page-subtitle">{t('absenceTypes.subtitle')}</p>
          </div>
          <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
            <button id="absence-types-filter-toggle" className={`filter-toggle-btn${showFilters ? ' active' : ''}`} onClick={() => setShowFilters(v => !v)}>
              <span className="filter-toggle-icon" style={{ transform: showFilters ? 'rotate(180deg)' : 'rotate(0deg)' }}>⌄</span>
              {t('common.filters')}
            </button>
            <Button fillMode="outline" themeColor="base" title={t('common.exportExcel')} onClick={() => exportToExcel(data, 'absence-types')} style={{ minWidth: 36, height: 36, padding: '0 10px' }}>📥</Button>
            <Button id="absence-type-add" themeColor="primary" fillMode="solid" onClick={() => navigate('/absence-types/new')}>+ {t('absenceTypes.addButton')}</Button>
          </div>
        </div>
          ...
          <div className="filter-accordion-inner">
            ...
            <div className="filter-bar-fields">
              <div className="filter-field">
                <label className="filter-label">{t('absenceTypes.columns.name') || 'Name'}</label>
                <Input value={filterName} onChange={e => setFilterName(String(e.value ?? ''))} placeholder={t('absenceTypes.columns.name') || 'Name'} style={{ width: '100%' }} />
              </div>
              <div className="filter-field">
                <label className="filter-label">{t('absenceTypes.columns.status') || 'Status'}</label>
      ...
      <div className="section-card" style={{ position: 'relative' }}>
        <div className="section-card-body no-pad" style={{ position: 'relative' }}>
          ...
          >
            <GridColumn field="idAbsenceType" title={t('absenceTypes.columns.id') || 'ID'} width="80px" />
            <GridColumn field="name" title={t('absenceTypes.columns.name') || 'Name'} />
            <GridColumn field="status" title={t('absenceTypes.columns.status') || 'Status'} width="130px" cells={{ data: (p) => <StatusCell dataItem={p.dataItem} t={t} /> }} />
      ...
      {itemToDelete && (
        <DeactivateDialog
          ...
          title={t('absenceTypes.actions.confirmDeleteTitle', 'Delete Absence Type')}
          message={t('absenceTypes.actions.confirmDeleteMessage', `Are you sure you want to delete ${itemToDelete.name}?`)}
        />
      )}
    </div>
  );
