...
const RolesContainer = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await RoleService.getRoles(currentPage, page.take);
    ...
    } catch { toastError(t('roles.title') + ': error loading data'); }
  ...
  const handleEdit   = (item: any) => navigate(`/roles/${item.idRole ?? item.id}/edit`);
  ...
  const confirmDelete = async () => {
    ...
    try { 
      // Replace with RoleService.deleteRole if implemented
    ...
    <div className="page-content animate-fade-in">
      <div className="page-header-card">
        <div className="page-header-card-top">
          <div>
            <div className="page-title"><div className="page-title-icon">🛡️</div>{t('roles.title')}</div>
            <p className="page-subtitle">{t('roles.subtitle')}</p>
          </div>
          <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
            <button id="roles-filter-toggle" className={`filter-toggle-btn${showFilters ? ' active' : ''}`} onClick={() => setShowFilters(v => !v)}>
              <span className="filter-toggle-icon" style={{ transform: showFilters ? 'rotate(180deg)' : 'rotate(0deg)' }}>⌄</span>
              {t('common.filters')}
            </button>
            <Button fillMode="outline" themeColor="base" title={t('common.exportExcel')} onClick={() => exportToExcel(data, 'roles')} style={{ minWidth: 36, height: 36, padding: '0 10px' }}>📥</Button>
            <Button id="role-add" themeColor="primary" fillMode="solid" onClick={() => navigate('/roles/new')}>+ {t('roles.addButton')}</Button>
          </div>
        </div>
          ...
          <div className="filter-accordion-inner">
            ...
            <div className="filter-bar-fields">
              <div className="filter-field">
                <label className="filter-label">{t('roles.columns.name') || 'Name'}</label>
                <Input value={filterName} onChange={e => setFilterName(String(e.value ?? ''))} placeholder={t('roles.columns.name') || 'Name'} style={{ width: '100%' }} />
              </div>
              <div className="filter-field">
                <label className="filter-label">{t('roles.columns.status') || 'Status'}</label>
      ...
      <div className="section-card" style={{ position: 'relative' }}>
        <div className="section-card-body no-pad" style={{ position: 'relative' }}>
          ...
          >
            <GridColumn field="idRole" title={t('roles.columns.id') || 'ID'} width="80px" />
            <GridColumn field="name"   title={t('roles.columns.name') || 'Name'} />
            <GridColumn field="status" title={t('roles.columns.status') || 'Status'} width="130px" cells={{ data: (p) => <StatusCell dataItem={p.dataItem} t={t} /> }} />
      ...
      {itemToDelete && (
        <DeactivateDialog
          ...
          title={t('roles.actions.confirmDeleteTitle', 'Delete Role')}
          message={t('roles.actions.confirmDeleteMessage', `Are you sure you want to delete ${itemToDelete.name}?`)}
        />
      )}
    </div>
  );
